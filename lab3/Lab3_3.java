...
Solves assignment 3 in LAB3

Implements binary search tree.
Asks the user for a range of the frequency rankings they want and returns it.
...
public class Lab3_3<Key extends Comparable<Key>, Value extends Comparable<Value>>{
    //BST
    ...
    public void rankN(int n, int x){                            //Function to return a certain range of frequency ranking
        Lab3_3<Key, Value> st = new Lab3_3<Key, Value>();       //copy BST
        ...
        for(int i = 0; i < x; i++){                             //only do this for the upper value of the range
            for(Key key : this.keys()){                         //iterate through original BST
                if(iteKey == null && !st.contains(key)){        //Update temp Key and temp Value to the root's key and value if copy BST does not store the node
                    iteKey = key;
                    iteVal = this.getBst(key);
                }
                if(iteVal.compareTo(this.getBst(key)) < 0 && !st.contains(key)){//Update temp Key and temp Value if the current node has greater value
                    iteKey = key;                                               //and the copy BST does not store the current node
                    iteVal = this.getBst(key);                                  //this will return the greatest value node on first iteration, next greatest value node on the second and so on
                }
            }
	...
	public static void main(String[] args) {
		...
		Lab3_3<String, Integer> st = new Lab3_3<String, Integer>();
